...
sudo docker restart "ppt-$mode-$player" && sudo docker exec -d "ppt-$mode-$player" python /home/seluser/scripts/ppt.py "$baseURL$player/?id=$id&mode=$mode" "$duration" "$mode"
